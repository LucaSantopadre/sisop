...
int main(int argc, char** argv){
	...
	path_name = argv[1];//default assignemnt to main threa for I/O upon SIGINT

	signal(SIGINT,print);

	sd = semget(IPC_PRIVATE,2,IPC_CREAT|IPC_EXCL|0666);
	if(sd == -1){
	...
	for (i = 0; i < num_threads; i++){
		ret = pthread_create(&tid,NULL,worker,argv[i+1]);
		if(ret == -1){
