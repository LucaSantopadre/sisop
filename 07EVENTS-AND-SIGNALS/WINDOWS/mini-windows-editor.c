...
LRESULT CALLBACK WndProc(HWND hWindow, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	if (message == WM_COMMAND){
		printf("process asked to run some menu/command - param is %d\n", wParam);
...
void main(int argc, char *argv[])
{
	...
	file = CreateFile(argv[1], GENERIC_READ|GENERIC_WRITE, 0, NULL, CREATE_NEW|OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	...
	AppendMenu(hMenu, MF_STRING, 128, "menu A");

	AppendMenu(hMenu, MF_STRING, 129, "menu B");
	...
	while (ret = GetMessage(&msg, NULL, 0, 0)) {
		...
		VERBOSE printf("got event-message while polling - type is %u\n", msg.message);
